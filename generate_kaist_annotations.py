...
# Load annotations from instances_train_annotations.txt
annotations_file_path = 'datasets/kaist-rgbt/annotations/instances_train_annotations.txt'
annotations = {}
...
def convert_to_kaist(data, annotations, categories):
    ...
    for idx, line in enumerate(data):
        image_id = line.strip().split('/')[-1]  # Extract image ID from path
        image_info = {
            "id": idx + 1,
            "file_name": line.strip(),  # Use the full path from the dataset
