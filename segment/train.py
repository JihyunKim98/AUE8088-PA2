...
def main(opt, callbacks=Callbacks()):
    ...
    # Evolve hyperparameters (optional)
        ...
        meta = {
            "lr0": (1, 1e-4, 1e-2),  # initial learning rate (SGD=1E-2, Adam=1E-3)   # 240614 1e-5 1e-2
            "lrf": (1, 0.01, 0.2),  # final OneCycleLR learning rate (lr0 * lrf)     # 0.1 -> 0.2
            "momentum": (0.9, 0.8, 0.98),  # SGD momentum/Adam beta1                 # 0.3 0.6 0.98
            "weight_decay": (1, 1e-5, 1e-3),  # optimizer weight decay
            "warmup_epochs": (1, 1.0, 3.0),  # warmup epochs (fractions ok)
            "warmup_momentum": (0.8, 0.6, 0.95),  # warmup initial momentum
            "warmup_bias_lr": (0.1, 0.05, 0.2),  # warmup initial bias lr
            "box": (0.05, 0.02, 0.1),  # box loss gain
            "cls": (0.5, 0.2, 2.0),  # cls loss gain
            "cls_pw": (1.0, 0.5, 1.5),  # cls BCELoss positive_weight
            "obj": (1.0, 0.5, 1.5),  # obj loss gain (scale with pixels)
            "obj_pw": (1.0, 0.5, 1.5),  # obj BCELoss positive_weight
            "iou_t": (0.2, 0.1, 0.5),  # IoU training threshold
            "anchor_t": (4.0, 2.0, 6.0),  # anchor-multiple threshold
            "anchors": (6, 2.0, 8.0),  # anchors per output grid (0 to ignore)
            "fl_gamma": (0.0, 0.0, 2.0),  # focal loss gamma (efficientDet default gamma=1.5)
            "hsv_h": (0.015, 0.0, 0.05),  # image HSV-Hue augmentation (fraction)
            "hsv_s": (0.7, 0.5, 0.9),  # image HSV-Saturation augmentation (fraction)
            "hsv_v": (0.4, 0.3, 0.7),  # image HSV-Value augmentation (fraction)
            "degrees": (10, 0.0, 30.0),  # image rotation (+/- deg)
            "translate": (0.1, 0.0, 0.3),  # image translation (+/- fraction)
            "scale": (0.5, 0.5, 1.0),  # image scale (+/- gain)
            "shear": (10, 0.0, 20.0),  # image shear (+/- deg)
            "perspective": (0.0, 0.0, 0.001),  # image perspective (+/- fraction), range 0-0.001
            "flipud": (0.0, 0.0, 0.5),  # image flip up-down (probability)
            "fliplr": (0.5, 0.0, 1.0),  # image flip left-right (probability)
            "mosaic": (1.0, 0.5, 1.0),  # image mixup (probability)
            "mixup": (0.0, 0.0, 1.0),  # image mixup (probability)
