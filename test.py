import os
import json
from sklearn.model_selection import train_test_split

# Load your dataset and replace {} with the appropriate value
dataset_path = 'datasets/kaist-rgbt/train-all-04.txt'
with open(dataset_path, 'r') as file:
    data = file.readlines()

# Replace {} with 'lwir' or 'visible' in the image paths
data = [line.replace("{}", "lwir") for line in data]  # Change to 'visible' if needed

# Load annotations from annotations.txt
annotations_file_path = 'datasets/kaist-rgbt/annotations/instances_val_annotations.txt'
annotations = {}
with open(annotations_file_path, 'r') as file:
    for line in file:
        parts = line.strip().split()
        image_id = parts[0]
        category_id = int(parts[1])
        bbox = [int(parts[2]), int(parts[3]), int(parts[4]), int(parts[5])]
        if image_id not in annotations:
            annotations[image_id] = []
        annotations[image_id].append({
            "category_id": category_id,
            "bbox": bbox,
            "area": bbox[2] * bbox[3],  # width * height
            "iscrowd": 0
        })

# Split the dataset into training and validation sets
train_data, val_data = train_test_split(data, test_size=0.2, random_state=42)

# Define a function to convert the dataset to KAIST format
def convert_to_kaist(data, annotations, categories):
    images = []
    anns = []
    for idx, line in enumerate(data):
        image_path = line.strip()
        image_id = os.path.basename(image_path)  # Extract image ID from path
        image_info = {
            "id": idx + 1,
            "file_name": image_path,  # Use the full path from the dataset
            "height": 512,  # Replace with actual height if available
            "width": 640   # Replace with actual width if available
        }
        images.append(image_info)

        if image_id in annotations:
            for ann in annotations[image_id]:
                ann_info = {
                    "id": len(anns) + 1,
                    "image_id": idx + 1,
                    "category_id": ann["category_id"],
                    "bbox": ann["bbox"],
                    "area": ann["area"],
                    "iscrowd": ann["iscrowd"]
                }
                anns.append(ann_info)
    
    kaist_format = {
        "images": images,
        "annotations": anns,
        "categories": categories
    }
    return kaist_format

# Define categories
categories = [
    {
        "id": 1,
        "name": "person",
        "supercategory": "none"
    },
    {
        "id": 2,
        "name": "cyclist",
        "supercategory": "none"
    },
    {
        "id": 3,
        "name": "people",
        "supercategory": "none"
    },
    {
        "id": 4,
        "name": "person?",
        "supercategory": "none"
    }
]

# Convert val data to KAIST format for evaluation
kaist_json_path = 'datasets/kaist-rgbt/annotations/KAIST_annotation.json'
val_kaist = convert_to_kaist(val_data, annotations, categories)
os.makedirs(os.path.dirname(kaist_json_path), exist_ok=True)

with open(kaist_json_path, 'w') as val_file:
    json.dump(val_kaist, val_file)

print(f"KAIST JSON file created: {kaist_json_path}")

# Validate JSON file
def validate_data(data):
    if 'images' not in data or 'annotations' not in data:
        return False, "Missing 'images' or 'annotations' key in JSON"
    
    for img in data['images']:
        if 'file_name' not in img or not os.path.exists(img['file_name']):
            return False, f"Image path {img['file_name']} does not exist"
    
    return True, "Validation successful"

# Load and validate the generated JSON file
with open(kaist_json_path, 'r') as f:
    data = json.load(f)

is_valid, message = validate_data(data)
print(message)

if is_valid:
    print("JSON 파일을 업로드할 준비가 되었습니다.")
else:
    print("JSON 파일 검증에 실패했습니다. 파일을 수정하세요.")
